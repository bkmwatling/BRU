...
static bool *memoise_in(BruStateMachine *sm)
{
    size_t        i, n, nstates = bru_smir_get_num_states(sm);
    size_t       *in_degrees = calloc(nstates, sizeof(*in_degrees));
    bool         *memo_sids  = calloc(nstates, sizeof(*memo_sids));
...
static void
cn_dfs(BruStateMachine *sm, bru_state_id sid, bool *has_backedge, bool *on_path)
{
    bru_trans_id *out_transitions;
    size_t        i, n;
    bru_state_id  dst;
...
static bool *memoise_cn(BruStateMachine *sm)
{
 ...
 * indexed by state identifier. A value of true in the array for a given state
 * identifier `i` indicates that state should be memoised, while a value of
 * false indicates it should not be.
 *
 ...
 */
static void memoise_states(BruStateMachine *sm, bool *sids, FILE *logfile)
{
