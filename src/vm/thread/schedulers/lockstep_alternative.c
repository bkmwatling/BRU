...
#include <bru/vm/thread/schedulers/lockstep_alternative.h>
...
BruScheduler *bru_lockstep_alt_scheduler_new(BruThreadManager *tm)
{
    ...
    stc_vec_default_init(&las->locked);

    las->active = NULL;
    stc_vec_default_init(&las->stack);
...
bru_lockstep_alt_scheduler_remove_low_priority_threads(BruScheduler *self)
{
    ...
    if (las->active || !stc_vec_is_empty(las->stack)) {
        ...
        if (las->active) stc_vec_push_back(&threads, las->active);
    }
                                    ...
                                    BruThread          *thread)
{
    ...
    for (i = 0;
         i < len && !(eq = bru_tm_check_thread_eq(tm, threads[i], thread));
         i++);
